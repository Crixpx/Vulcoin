...
#
# run with a mounted directory for ~/.VulCoin:
#   docker run -it -p 15015:15015 -v /path/to/a/local/directory:/root/.VulCoin moneybagcoin
#
...
#    docker run -it -p 15015:15015 -p 5000:XXX -v /path/to/a/local/directory:/root/.VulCoin moneybagcoin # Replace XXX with set rpcPort in moneybagcoin.conf
...
RUN git clone https://github.com/VulCoin/VulCoin.git VulCoin && cd VulCoin/src/ && make -f makefile.unix && cp moneybagcoind /usr/local/sbin/moneybagcoind

RUN mkdir /root/.VulCoin/
